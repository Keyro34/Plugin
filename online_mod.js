...
(function () {
    ...
    function isSameMovie(item, movie) {
        var title = normalize(item.title);
        var movieTitle = normalize(movie.title || movie.name);

        var orig = normalize(item.original_title);
        ...
        return (
            (title === movieTitle || orig === movieOrig) &&
            year == movieYear
        );
    }
    ...
    function findBestMatch(items, movie) {
        ...
        // Фильтруем ТОЛЬКО совпадающие фильмы
        items = items.filter(function(i){
            return isSameMovie(i, movie);
        });

        // Если нет совпадений → сразу результат
