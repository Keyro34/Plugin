...
(function () {
    ...
    function lumex(component, _object) {
      ...
      function append(items) {
          ...
          items.forEach(function (element) {
              ...
              // Формируем название серии (как на фото: "Пилот", "Полученные танцоры" и т.д.)
              var episodeTitle = element.title || 'Серия ' + episode_num;
              
              // Формируем информацию об озвучке (как на фото: "Неизвестно")
              var voiceText = element.translate_voice || 'Неизвестно';
              
              // Создаем hash для timeline
              ...
              // Создаем карточку как на фото
              var item = $('<div class="episode-card selector">' +
                  '<div class="episode-card__poster">' +
                      '<img src="" alt="">' +
                      '<div class="episode-card__loader"></div>' +
                      (viewed.indexOf(hash_file) !== -1 ? '<div class="episode-card__watched">✓</div>' : '') +
                  '</div>' +
                  '<div class="episode-card__content">' +
                      '<div class="episode-card__header">' +
                          '<div class="episode-card__title">' + episodeTitle + '</div>' +
                          '<div class="episode-card__time">' + timeFormatted + '</div>' +
                      '</div>' +
                      '<div class="episode-card__timeline"></div>' +
                      '<div class="episode-card__footer">' +
                          (rating ? '<div class="episode-card__rating">⭐ ' + rating + '</div>' : '') +
                          '<div class="episode-card__voice">' + voiceText + '</div>' +
                      '</div>' +
                  '</div>' +
              '</div>');

              // Добавляем timeline
              item.find('.episode-card__timeline').append(Lampa.Timeline.render(view));

              // Загружаем изображение
              var img = item.find('img')[0];
              var loader = item.find('.episode-card__loader');
              var posterDiv = item.find('.episode-card__poster');
              
              if (img) {
                  // Путь к изображению (постер эпизода)
                  var imagePath = element.still_path || element.poster_path || object.movie.poster_path;
                  
                  img.onload = function() {
                      posterDiv.addClass('episode-card__poster--loaded');
                      loader.remove();
                  };
                  
                  img.onerror = function() {
                      posterDiv.addClass('episode-card__poster--error');
                      loader.remove();
                      // Добавляем заглушку с номером серии
                      if (!posterDiv.find('.episode-card__poster-fallback').length) {
                          posterDiv.append('<div class="episode-card__poster-fallback">' + 
                              ('0' + episode_num).slice(-2) + '</div>');
                      }
                  };

                  if (imagePath) {
                      img.src = 'https://image.tmdb.org/t/p/w185' + imagePath;
                  } else {
                      posterDiv.addClass('episode-card__poster--error');
                      loader.remove();
                      posterDiv.append('<div class="episode-card__poster-fallback">' + 
              ...
              item.on('hover:enter', function (event, options) {
                  ...
                  getStream(element, function (element) {
                      ...
                      if (viewed.indexOf(hash_file) == -1) {
                          ...
                          if (!item.find('.episode-card__watched').length) {
                              item.find('.episode-card__poster').append('<div class="episode-card__watched">✓</div>');
                          }
    ...
    function addStyles() {
        ...
        .episode-card__poster--loaded img {
            opacity: 1;
        }
        
        .episode-card__poster--error {
            background: linear-gradient(135deg, #2a2a2a, #1a1a1a);
        }
        
        .episode-card__loader {
            position: absolute;
            top: 50%;
            left: 50%;
            width: 2em;
            ...
            border-radius: 50%;
            animation: episode-spin 0.8s linear infinite;
        }
        ...
        .episode-card__poster--loaded .episode-card__loader {
            display: none;
        }
        
        .episode-card__poster-fallback {
            position: absolute;
            top: 0;
            left: 0;
            right: 0;
        ...
        .episode-card__timeline {
            margin: 0.4em 0;
            width: 100%;
        }
        
        .episode-card__timeline .time-line {
        ...
        .episode-card__timeline .time-line__progress {
            height: 100%;
            background: #ffd700;
            border-radius: 0.1em;
        }
