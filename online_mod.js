    ...
    function rezka2(component, _object) {
      ...
      this.search = function (_object, kinopoisk_id, data) {
        ...
        var display = function display(links, have_more, query) {
            ...
            // НОВОЕ: Логи для отладки, чтобы видеть scores
            console.log('Best score:', best ? best.score : 'none');
            console.log('Second score:', second ? second.score : 'none');
            console.log('Diff between best and second:', best && second ? (best.score - second.score) : 'n/a');
            ...
            if(best){
                ...
                // Динамический порог (как был)
                var autoThreshold = 300;
                if(inputYear) autoThreshold += 40;
                if(mainOriginal) autoThreshold += 30;

                // НОВОЕ: Более строгая проверка "близости"
                // Если diff < 50 (или настройте под себя) ИЛИ score < 350, считаем "не близко" и показываем варианты
                var minDiffForAuto = 50;  // Новый параметр: минимальная разница для авто-выбора
                var minScoreForAuto = 350;  // Новый параметр: минимальный score для авто-выбора

                if(diff12 >= minDiffForAuto && best.score >= minScoreForAuto){
                    getPage(best.link);
                    return;
                }
                ...
                // Убрал третью проверку, чтобы чаще показывать варианты, если scores близки
                // Если хотите оставить, раскомментируйте:
                // if(third){
            ...
            // ================= FALLBACK =================
            // Если "не близко" (по новым условиям), показываем список для выбора
            component.similars(items);
            component.loading(false);
         };
